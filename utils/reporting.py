...
def generate_sdd_markdown(state: dict) -> str:
    ...
    if sim:
        ...
        md.append(
            f"- Tickets: **{sim.get('tickets','?')}** — "
            f"Tokens/ticket: **{sim.get('tokens_por_ticket','?')}**\n"
        )
        md.append(f"- Coste TI: **{sim.get('coste_ti',0):.2f} €**\n")
        md.append(f"- Coste Operativo: **{sim.get('coste_operativo',0):.2f} €**\n")
    ...
    md.append(
        "- Riesgo: Optimización TI (€/tokens) destruye SI (retrabajo humano). "
        "Mitigación: operar por **coste total** + FCR.\n"
    )
    md.append(
        "- Riesgo: PII en logs/prompts. "
        "Mitigación: filtro PII + minimización + auditoría.\n"
    )
    md.append(
        "- Riesgo: Sin kill switch / sin HITL. "
        "Mitigación: control operativo (SRE/Owner) y escalado.\n"
    )
...
def markdown_to_pdf_bytes(md: str) -> bytes:
    ...
    for raw in md.splitlines():
        ...
        if line.startswith("# "):
            flush_bullets()
            story.append(
                Paragraph(_md_inline_to_html(line[2:]), styles["Title"])
            )
        ...
        flush_bullets()
        story.append(
            Paragraph(_md_inline_to_html(line), styles["BodyText"])
        )
        story.append(Spacer(1, 6))
